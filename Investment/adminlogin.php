...
<?php 
    ...
    if (isset($_POST['login'])){
       ...
       $query = "SELECT id FROM admin WHERE username = '$user' AND password = '$realpass'";
            $result = mysqli_query ($link,$query);
            if($result){
                if(mysqli_num_rows($result) ==1){
                   $_SESSION['user'] = $user;
                    $_SESSION['password1'] = $realpass; 
                   header("location:admin.php");
                    
                    echo 'Access granted!';
                }else{
                    $error1 = "<strong><p style = \"color:red\">Forbidden login attempt! Developer will be notified</p></strong>";
                }
            }
...
<head>
  ...
  <title>Fortfolio || Admin login</title>
  ...
  <section class="w3l-contact-11 py-5">
    <div class="contacts-main py-lg-5 py-md-4">
      <div class="title-content text-center">
        <h3 class="hny-title" span style="color: red">Admin login</h3>
    </div>
            ...
            <div class="form-inner-cont">
              <form action="adminlogin.php" method="post" class="signin-form">
                <div class="form-input">
                  <label for="w3lName">Admin Username</label>
                  <input type="text" name="user" id="w3lName" placeholder="" />
                </div>
                <div class="form-input">
